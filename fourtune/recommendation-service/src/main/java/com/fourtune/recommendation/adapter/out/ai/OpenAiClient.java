 ...
 * 프로덕션 환경에서 사용. feature.ai.provider=openai 일 때 활성화.
 */
@Slf4j
@Component
...
public class OpenAiClient implements RecommendationAiClient {
    ...
    public List<RecommendedItemResponse> recommend(Long userId, List<String> topCategories,
        ...
        log.info("[REC][AI] OpenAI 추천 시작: userId={}, 후보={}개, 카테고리={}",
        ...
        try {
            ...
            log.error("[REC][AI] OpenAI 호출 실패, 원본 순서 반환: {}", e.getMessage());
            return candidates;
        }
    }
    ...
    private List<RecommendedItemResponse> parseResponse(String response,
        ...
        try {
            ...
            log.info("[REC][AI] OpenAI 추천 완료: 재정렬 {}개", ranked.size());
            return ranked;
